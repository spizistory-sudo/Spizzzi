...
async function main() {
  ...
  const biblePath = path.join(__dirname, '..', 'docs', 'library-bible-v2.md');
  if (!fs.existsSync(biblePath)) {
    console.error(`Bible file not found: ${biblePath}`);
    process.exit(1);
  }
  const content = fs.readFileSync(biblePath, 'utf-8');
  console.log(`Read bible: ${content.length} chars, ${content.split('\n').length} lines`);
  ...
  if (existing && existing.length > 0) {
    ...
    const { error } = await db
      .from('library_knowledge')
      .update({ content, title: 'Writing Bible v2' })
      .eq('id', existing[0].id);
