  ...
  function renderSpread(v: number) {
    ...
    if (v >= FIRST_STORY && v <= LAST_STORY) {
      ...
      return (
        ...
        {/* ── Mobile: full-bleed image + text band ── */}
        <div className="lg:hidden w-full h-full relative overflow-hidden">
          ...
          {(() => {
            ...
            return page.illustration_url ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #0A1128, #1a1035)' }}>
          ...
          {/* Text band — pinned to bottom, capped height, scrollable */}
          <div className="absolute left-0 right-0 bottom-0 z-10 text-scroll" style={{ maxHeight: '42vh', overflowY: 'auto', WebkitOverflowScrolling: 'touch', background: 'rgba(10,16,40,0.80)', padding: '14px 18px', scrollbarWidth: 'thin', scrollbarColor: 'rgba(155,125,212,0.3) transparent' }}>
            {isEditMode ? (
              <textarea
                ...
                style={{ fontFamily: 'Georgia, serif', fontSize: '16px', lineHeight: 1.7, color: '#F4EFE2', minHeight: '3em' }}
                placeholder="Enter story text..."
              />
            ) : (
              <p style={{ color: '#F4EFE2', fontSize: '16px', lineHeight: 1.7, fontFamily: 'Georgia, serif' }} dir="auto">{page.text_content}</p>
            )}
            <p className="mt-2" style={{ fontSize: '11px', color: 'rgba(255,255,255,0.25)', textAlign: 'center' }}>{page.page_number}</p>
          </div>
        </div>

        {/* ── Desktop: 50/50 spread (unchanged) ── */}
            ...
            <div className="text-scroll" style={{ position: 'absolute', top: '4rem', bottom: '3rem', left: '3rem', right: '3rem', overflowY: 'auto', overflowX: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', scrollbarWidth: 'thin', scrollbarColor: 'rgba(155,125,212,0.4) transparent' }}>
              {isEditMode ? (
                <textarea
                  ...
                  onChange={(e) => handleTextChange(page.id, page.text_content, e.target.value)}
                  dir="auto"
                  style={{ width: '100%', height: '100%', background: 'transparent', border: 'none', borderBottom: '1px solid rgba(100,80,40,0.30)', outline: 'none', resize: 'none', fontFamily: 'Georgia, serif', fontSize: '20px', lineHeight: 1.8, color: '#1f2937', textAlign: 'center', cursor: 'text', padding: '4px 2px' }}
                  placeholder="Enter story text..."
                />
              ) : (
