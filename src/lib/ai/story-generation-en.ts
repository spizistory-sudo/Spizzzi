...
- "action": one sentence describing the single key visual moment — the gesture, exchange, or movement to illustrate. Include key props. If the page has multiple beats, pick the one with the most character interaction. Example: "Inés hands Tani a piece of golden-brown sweet bread (concha); Tani reaches for it, both smiling." IMPORTANT: if a secondary character is in characters_in_frame, the action MUST involve them interacting with the protagonist (handing, pointing, teaching, laughing together, etc.) — never describe the protagonist acting alone when another character is present.

The illustration_brief is the image model's primary instruction. Make it vivid and specific. The image model tends to drop secondary characters — so be explicit about WHO is in the frame and WHAT they are doing together.
